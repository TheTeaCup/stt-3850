...
CLASS_factor <- factor(CLASS,ordered = TRUE, levels = c(“freshman”,“sophomore”,“junior”,“senior”))
...
library(ggplot2)
ggplot(iris,aes(y=Sepal.Width,x=factor(Species)))+geom_boxplot()
...
IRIS_graph <- ggplot(iris,aes(x=Petal.Length,y=Petal.Width))+geom_point()
IRIS_graph
...
library(dplyr)
iris2 <- iris %>% filter(Species =="setosa" | Species == "versicolor") %>% select(Petal.Length,Petal.Width,Species)
...
iris2 %>% group_by(Species) %>% summarize(IQR = IQR(Petal.Length))
...
ggplot(iris2,aes(x=Petal.Length))+geom_histogram(color="white")+facet_wrap(iris2$Species)
...
df <- data.frame(x,f)
library(ggplot2)
ggplot(df,aes(x=x,y=f))+geom_point()+geom_segment(x = 12, y =0, xend = 12, yend = 1/36)+annotate("rect",xmin = 0, xmax=12, ymin = 0, ymax= 1/36)
